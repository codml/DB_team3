<template>
    ...
    <div class="findPW">
      ...
      <form @submit.prevent="handleFind">
        ...
        <div class="button-container">
          <button class="fade-button" type="submit">패스워드 찾기</button>
        </div>
      </form>    
    </div>
    <br>
  ...
  export default {
    ...
    methods: {
      async handleFind() {
        try {
          if (this.form.userID && this.form.userEmail) {
            console.log('아이디 찾기:', this.form.userID, this.form.userEmail);
            const response = await axios.post('http://localhost:3000/account/password', this.form);
            console.log('서버 응답:', response.data.message);

            if (response.data.message === 'success') {
              alert('비밀번호가 회원님의 이메일로 성공적으로 전송되었습니다.\n메일을 확인해주세요.');
